...
if curl -fsSI --max-time 2 "${URL}" 2>/dev/null | grep -qi '^X-45OJuke-Server: no-cache-v1'; then
  echo "Le serveur sans cache est deja disponible."
  open "${URL}"
  exit 0
fi
